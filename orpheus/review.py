...
from . import arrangement, media, projects, observability as obs


def candidate(case, candidate_id):
    if not isinstance(candidate_id, str) or not __import__('re').fullmatch(r'[a-f0-9]{12}', candidate_id):
    ...
    if hashlib.sha256(audio.read_bytes()).hexdigest() != result['audio_sha256']:
        raise ValueError('Candidate audio has changed. Render a new alternative before review.')
    return result
...
def save_review(data):
    ...
    record = {
        'candidate_id': rendered['id'], 'audio_sha256': rendered['audio_sha256'],
        'verdict': verdict, 'note': note, 'provenance': 'human_ui', 'created_at': time.time(),
    }
    projects.atomic(projects.project_dir(case['id']) / (uuid.uuid4().hex[:12] + '-human.json'), record)
    obs.emit(case['id'], 'human_review', {**record, 'parent_project_id': case.get('take_parent_project_id', case['id'])})
    return record
...
def assist(data):
    ...
    rows = arrangement.validate(
        data['rows'], target_duration_s=case['seconds'],
    ...
    revised = {
        **bound, 'rows': rows, 'summary': arrangement.suitability(rows),
        'provenance': 'human_assisted', 'human_approved': False,
        'parent_candidate_id': rendered['id'], 'id': uuid.uuid4().hex[:20],
    }
    folder = projects.project_dir(case['id'])
    result = arrangement.render(case, revised, folder)
    result.update(provenance='human_assisted', cycle=0, hypothesis='User edited mapping; not autonomous evaluation')
