...
from . import config, media, projects, review, takes, observability as obs
    ...
    with LOCK:
        ...
        with (projects.project_dir(project_id) / 'worker.log').open('ab') as output:
            PROCESS = subprocess.Popen(
                [sys.executable, '-m', 'orpheus.worker', project_id, '--feedback', feedback],
                cwd=config.ROOT, stdout=output, stderr=output, start_new_session=True,
            )


def project_list():
    rows, errors = [], []
    for path in sorted(projects.PROJECTS.glob('*/project.json'), key=lambda p: p.stat().st_mtime, reverse=True):
        try:
            doc = json.loads(path.read_text())
            doc['turn_details'] = [json.loads((path.parent / (tid + '-turn.json')).read_text())
                                   for tid in doc['turns'] if (path.parent / (tid + '-turn.json')).exists()]
            doc['assisted_candidates'] = [json.loads(p.read_text()) for p in sorted(path.parent.glob('*-assisted.json'))]
            doc['human_reviews'] = [json.loads(p.read_text()) for p in sorted(path.parent.glob('*-human.json'))]
            rows.append(doc)
        except (ValueError, KeyError, OSError):
            errors.append({'project_id': path.parent.name, 'error': 'Project receipt is unreadable; files retained.'})
...
def public_config():
    ...
    return {
        'max_file_bytes': config.MAX_FILE_BYTES, 'max_duration_s': config.MAX_DURATION_S,
        'max_brief_chars': config.MAX_BRIEF_CHARS, 'max_feedback_chars': config.MAX_FEEDBACK_CHARS,
        'max_controller_calls': config.MAX_CONTROLLER_CALLS,
        'audio_enabled': config.AUDIO_ENABLED, 'controller_models': list(MODELS),
        'audio_model': MODEL, 'storage': 'local',
...
def upload_files(form, directory, names):
    ...
    for field in names:
        ...
        if not 0 < len(data) <= config.MAX_FILE_BYTES:
            raise RequestError('Each file must be nonempty and within the configured limit.', 413)
...
class Handler(LocalHandler):
    ...
    def do_GET(self):
        try:
            ...
            self.send_json({'error': 'Invalid project, candidate or media window.'}, 400)
        except (OSError, subprocess.SubprocessError):
            self.send_json({'error': 'Local media is unavailable. Check the project files and FFmpeg.'}, 422)
    ...
    def get_route(self):
        ...
        if route in ('/', '/workspace'):
            ...
            projects.load(pid)
            self.send_json({'takes': takes.list_takes(pid), 'experiments': [json.loads(p.read_text())
                            for p in sorted(projects.project_dir(pid).glob('*-experiment.json'))]})
    ...
    def project_file(self, relative):
        allowed = re.fullmatch(
            ...
            r'[a-f0-9]{12}\.(?:mp4|wav|json)|[a-f0-9]{12}-turn\.json|takes/[a-f0-9]{12}\.wav)', relative)
        if not allowed:
        ...
        if name == 'poster.jpg':
            ...
            with LOCK:
                if not poster.exists():
                    frames = projects.frames(case, [min(.5, case['seconds'] / 2)], projects.project_dir(pid) / 'frames')
    ...
    def audio_data(self, route, query):
        ...
        if route == '/api/snippet':
            ...
            if not np.isfinite([start, end]).all() or not 0 <= start < end <= len(source) / media.RATE:
            ...
            with wave.open(output, 'wb') as wav:
                wav.setparams((1, 2, media.RATE, len(samples), 'NONE', 'not compressed'))
                wav.writeframes((np.clip(samples, -1, 1) * 32767).round().astype('<i2').tobytes())
        ...
        self.send_json({'duration_s': len(samples) / media.RATE, 'sample_rate': media.RATE,
        ...
        try:
            ...
            if route in ('/api/projects', '/api/takes'):
                if not UPLOAD_LOCK.acquire(blocking=False):
                    raise RequestError('Another upload is being prepared. Retry when it finishes.', 409)
                try:
                    self.upload(route)
                finally:
                    UPLOAD_LOCK.release()
            ...
            self.send_json({'error': 'An agent run or edit is active. Wait for it to finish.'}, 409)
        except (ValueError, KeyError, TypeError, AttributeError, UnicodeError):
            self.send_json({'error': 'Invalid input. Check the selected project, values and file types.'}, 400)
        except FileNotFoundError:
            self.send_json({'error': 'Project or artifact not found.'}, 404)
        except (OSError, subprocess.SubprocessError):
            self.send_json({'error': 'Media operation failed. Completed files remain available; check FFmpeg and disk space.'}, 422)

    def upload(self, route):
        take = route == '/api/takes'
        allowed = {'project_id', 'audio', 'brief', 'start_s', 'clock'} if take else {'video', 'sfx', 'context', 'style'}
        form = self.read_form(config.MAX_FILE_BYTES * (1 if take else 2) + 20000, allowed)
        with tempfile.TemporaryDirectory(prefix='orpheus-upload-') as temporary:
            files = upload_files(form, Path(temporary), ['audio'] if take else ['video', 'sfx'])
            if take:
                with mutation():
                    result = takes.add_take(form['project_id'], files['audio'][0], form.get('brief', ''),
                                            float(form.get('start_s', '0')), form.get('clock', 'uploaded'))
            else:
                result = {'project': projects.create(files['video'][0], files['sfx'][0], form.get('context', ''),
                                                    form.get('style', ''), files['video'][1], files['sfx'][1])}
        self.send_json(result, 201)

    def json_action(self, route):
        if route not in ('/api/run', '/api/review', '/api/assist', '/api/takes/fit', '/api/grafana'):
            raise RequestError('Route not found.', 404)
        ...
        if route == '/api/run':
            ...
            self.send_json(asyncio.run(obs.investigate(data['project_id'], data.get('topic', 'history'), data.get('candidate_id', ''))))
        elif route == '/api/takes/fit':
            with LOCK:
                with mutation():
                    doc = takes.fitting_project(data['project_id'], data['take_id'])
                start(doc['id'], 'Query Grafana takes for the parent project. Compare recorded takes, propose one evidence-linked prop or performance experiment, then fit this selected take to picture and compare measurements through Grafana.')
            self.send_json({'project': doc}, 201)
        else:
            with mutation():
                result = review.save_review(data) if route == '/api/review' else review.assist(data)
            self.send_json(result, 201)
