...
from orpheus.arrangement import validate, suitability
...
class ArrangementTests(unittest.TestCase):
    ...
    def test_tail_options_preserve_baseline_landmark_and_bounds(self):
        ...
        self.assertLess(options[0]['source_range_s'][1], options[1]['source_range_s'][1])
    ...
    def test_automatic_impact_preparation(self):
        ...
        row = self.canonical([self.row(source_range_s=[0, 1], target_range_s=[1, 1.8])])[0]
    ...
    def canonical(self, rows):
        return validate(rows, target_duration_s=4, source_duration_s=2, target_ids={'t1', 't2'}, source_ids={'s1'})
    ...
    def test_composite_chronology_and_balanced_strength(self):
        ...
        row = self.canonical([self.row(kind='composite', anchor='start', source_range_s=[0, 0.6], target_range_s=[1, 1.6], target_body_dbfs=-22)])[0]
    ...
    def test_mixed_anchor_overlap_and_pause(self):
        source = np.zeros(96000)
        source[9600] = 0.6
        rows = self.canonical([self.row(target_range_s=[0.5, 1], source_range_s=[0.1, 0.4], source_anchor_s=0.2, target_anchor_s=0.7, fade_in_s=0, fade_out_s=0), self.row(id='second', target_range_s=[0.5, 1], source_range_s=[0.1, 0.4], source_anchor_s=0.2, target_anchor_s=0.7, fade_in_s=0, fade_out_s=0, reuse_reason='layer test', gain_db=-6), self.row(id='pause', target_id='t2', source_id='', target_range_s=[2, 3], source_range_s=[0, 0], kind='pause', disposition='omit')])
    ...
    def test_texture_no_implicit_repeat_and_weak_dynamics(self):
        rows = self.canonical([self.row(kind='texture', anchor='start', target_range_s=[0, 1], source_range_s=[0, 0.4])])
    ...
    def test_spike_shape_variable_strength_and_source_eof(self):
        ...
        source[9600] = 1
        raw = self.canonical([self.row(target_range_s=[0, 0.5], source_range_s=[0, 0.5], anchor='start', fade_in_s=0, fade_out_s=0)])
    ...
    def test_real_export(self):
        ...
        rows = self.canonical([self.row(target_range_s=[1, 1.3], source_range_s=[0.1, 0.4])])
    ...
    def row(self, **changes):
        row = {'id': 'm1', 'target_id': 't1', 'source_id': 's1', 'target_range_s': [1, 1.2], 'source_range_s': [0.1, 0.4], 'kind': 'impact', 'anchor': 'contact', 'disposition': 'use', 'confidence': 'uncertain', 'evidence': 'provisional timing evidence'}
        row.update(changes)
        return row

    def test_explicit_mapping_and_pause(self):
        rows = validate([self.row(), self.row(id='pause', target_id='t2', source_id='none', target_range_s=[2, 3], source_range_s=[0, 0], kind='pause', anchor='transition', disposition='omit')], target_duration_s=4, source_duration_s=2, target_ids={'t1', 't2'}, source_ids={'s1'})
    ...
    def test_render_ready_impact_requires_explicit_landmarks(self):
        with self.assertRaisesRegex(ValueError, 'explicit contact and source landmarks'):
            validate([self.row()], target_duration_s=4, source_duration_s=2, target_ids={'t1'}, source_ids={'s1'}, require_impact_anchors=True)
        ready = self.row(target_anchor_s=1.1, source_anchor_s=0.2)
        rows = validate([ready], target_duration_s=4, source_duration_s=2, target_ids={'t1'}, source_ids={'s1'}, require_impact_anchors=True)
    ...
    def test_rejects_invented_used_ids_and_unordered_rows(self):
        with self.assertRaises(ValueError):
            validate([self.row(source_id='invented')], target_duration_s=4, source_duration_s=2, target_ids={'t1'}, source_ids={'s1'})
        with self.assertRaises(ValueError):
            validate([self.row(id='late', target_range_s=[2, 2.1]), self.row(id='early', target_range_s=[1, 1.1])], target_duration_s=4, source_duration_s=2, target_ids={'t1'}, source_ids={'s1'})

    def test_range_and_confidence_guards(self):
        with self.assertRaises(ValueError):
            validate([self.row(target_range_s=[-1, 0])], target_duration_s=4, source_duration_s=2, target_ids={'t1'}, source_ids={'s1'})
        with self.assertRaises(ValueError):
            validate([self.row(confidence='verified')], target_duration_s=4, source_duration_s=2, target_ids={'t1'}, source_ids={'s1'})
