...
from orpheus.projects import project_dir, media, frames, ff, create, ROOT
from unittest.mock import patch
from orpheus import fitting
...
class Checks(unittest.TestCase):
    ...
    def test_ingest_bounds_and_silent_source(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            ff('-f', 'lavfi', '-i', 'color=c=blue:s=64x64:r=5', '-t', 31, '-c:v', 'libx264', video)
    ...
    def test_agent_gates(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            ctx = SimpleNamespace(state={}, actions=SimpleNamespace(escalate=False, skip_summarization=False))
            self.assertIn('error', tools['render_plan'](-22, 'balanced', 'test', ctx))
            self.assertIn('error', tools['render_texture'](0, 6, 0, 3, -22, True, 0.1, 'test', ctx))
            self.assertIn('error', tools['finish']('madeup', 'needs_human_review', 'test', [], ctx))
            sample = next((r['id'] for r in fitting.bank(case) if r['usable']))
            plan = [dict(time_s=1, duration_s=0.3, sample_id=sample, gain_db=0, confidence='likely', evidence='test')]
            ctx.state['inspected'] = True
            self.assertIn('error', tools['render_texture'](0, 6, 0, 3, -22, True, 0.1, 'unreviewed', ctx))
            self.assertIn('error', tools['save_event_plan'](json.dumps(plan), ctx))
            ctx.state['reviewed'] = [1]
            self.assertIn('error', tools['record_event_review'](1, 'target', 'Not delivered yet', ctx))
            ctx.state['delivered_centers'] = [1]
            self.assertEqual(tools['record_event_review'](1, 'target', 'Visible transition and local onset', ctx)['saved'], True)
            self.assertEqual(tools['save_event_plan'](json.dumps(plan), ctx)['saved_events'], 1)
            tools['record_review_batch'](json.dumps([dict(center_s=1, verdict='non_target', observation='Corrected interpretation')]), ctx)
            self.assertIn('error', tools['save_event_plan'](json.dumps(plan), ctx))
    ...
    def test_persistent_session(self):
        ...
        async def check(folder):
            ...
            s = await one.create_session(app_name='test', user_id='u', session_id='p', state={'notes': []})
            await one.append_event(s, Event(author='system', actions=EventActions(state_delta={'notes': ['keep subtle'], 'plan': [{'time_s': 1}]})))
            await one.close()
    ...
    def test_source_bank_and_bounds(self):
        ...
        bad = [{'time_s': 1, 'duration_s': 0.3, 'sample_id': -1, 'gain_db': 0, 'confidence': 'likely', 'evidence': 'test'}]
        with self.assertRaises(ValueError):
    ...
    def test_per_event_fit(self):
        ...
        plan = [dict(time_s=t, duration_s=0.3, sample_id=ids[i % len(ids)], gain_db=-i, confidence='uncertain', evidence='Synthetic schedule test') for i, t in enumerate([1.0, 2.0, 3.0])]
        with tempfile.TemporaryDirectory() as d:
    ...
    def test_cracker_peak_retained_after_compression(self):
        case = load_case('crackers')
        ids = [r['id'] for r in fitting.bank(case) if r['usable']]
        plan = [dict(time_s=2.0 + i * 2, duration_s=0.6, sample_id=sid, gain_db=0, confidence='uncertain', evidence='Synthetic codec regression') for i, sid in enumerate(ids)]
        with tempfile.TemporaryDirectory() as d:
            r = fitting.render(case, plan, Path(d), -22)
