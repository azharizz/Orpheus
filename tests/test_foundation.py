...
class Foundation(unittest.TestCase):
    ...
    def test_preparation_discloses_source_truncation_and_no_audio(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            projects.ff('-f', 'lavfi', '-i', 'color=s=64x64:r=5', '-t', 0.6, '-c:v', 'libx264', video)
            projects.ff('-f', 'lavfi', '-i', 'sine=frequency=300:sample_rate=48000', '-t', 31, sfx)
            ...
            for name, source in [('video', video), ('sfx', sfx)]:
                self.assertEqual((folder / 'projects' / p['id'] / p['preparation']['original_files'][name]).read_bytes(), source.read_bytes())
    ...
    def test_initialization_failure_is_persisted(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch.object(worker, 'load', return_value=doc), patch.object(worker, 'project_dir', return_value=folder), patch.object(worker, 'session_service', side_effect=RuntimeError('secret')):
                result = asyncio.run(worker.run_turn('a' * 16, 'test'))
            self.assertEqual(result['status'], 'failed')
            self.assertEqual(result['failure']['phase'], 'session_initialization')
            self.assertEqual(json.loads((folder / 'project.json').read_text())['status'], 'failed')
    ...
    def test_orphaned_turn_recovery(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            projects.atomic(folder / 'project.json', doc)
            with patch.object(worker, 'load', return_value=doc), patch.object(worker, 'project_dir', return_value=folder):
                worker.recover_orphaned_turns('a' * 16)
            self.assertEqual(json.loads((folder / ('b' * 12 + '-turn.json')).read_text())['status'], 'interrupted')
            self.assertEqual(json.loads((folder / 'project.json').read_text())['status'], 'interrupted')
    ...
    def test_cleanup_failure_is_persisted(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            folder = Path(d)
            doc = {'id': 'a' * 16, 'status': 'ready', 'turns': [], 'prepared_hashes': {}}
            projects.atomic(folder / 'project.json', doc)
            with patch.object(worker, 'load', return_value=doc), patch.object(worker, 'project_dir', return_value=folder), patch.object(worker, 'session_service', return_value=service):
                result = asyncio.run(worker.run_turn('a' * 16, 'test'))
    ...
    def test_oversized_summaries_are_rejected_before_work(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn('error', tools['render_plan'](-22, 'balanced', 'x' * 1001, ctx))
            self.assertIn('error', tools['finish']('', 'unsuitable', 'x' * 3001, [], ctx))
            self.assertIn('error', tools['finish']('', 'unsuitable', 'reason', ['x'] * 16, ctx))

    def test_failure_categories_do_not_include_exception_text(self):
        from orpheus.worker import failure_info
        from google.adk.agents.invocation_context import LlmCallsLimitExceededError
        for exc, phase, expected in [(LlmCallsLimitExceededError('secret'), 'agent', 'model_call_budget'), (TimeoutError('secret'), 'agent', 'turn_timeout'), (subprocess.TimeoutExpired('secret', 1), 'overview', 'media_timeout'), (subprocess.CalledProcessError(1, 'secret'), 'agent', 'media_processing'), (RuntimeError('secret'), 'agent', 'unexpected_failure')]:
            result = failure_info(exc, phase)
            self.assertEqual(result['category'], expected)
            self.assertNotIn('secret', str(result))
        self.assertEqual(failure_info(RuntimeError('secret'), 'agent', True)['category'], 'provider_failure')

    def test_inspection_discloses_limits(self):
        from orpheus.workflow_common import inspection_scope
        ...
        self.assertFalse(inspection_scope(2, 3, 1, 0)['candidate_cap_may_have_omitted_events'])
