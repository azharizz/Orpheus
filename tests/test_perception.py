...
class PerceptionTests(unittest.TestCase):
    ...
    def test_label_wording_is_not_a_conflict(self):
        ...
        def evidence(label, kind='texture'):
            ...
            return {'evidence_id': label, 'receipt': r, 'inventory': p.validate_inventory(doc, r), 'measurements': p.measurements(a, r)}
        ...
        self.assertIn('Overlapping prior hypothesis differs; correspondence unresolved', p.reconcile(new, [], [evidence('motor hum', 'composite')])['events'][0]['conflicts'])
    ...
    def test_source_reviews_use_source_audio_not_target_frames(self):
        ...
        ev = {'evidence_id': 'source-evidence', 'receipt': r, 'inventory': p.validate_inventory(self.document(r), r), 'measurements': p.measurements(a, r)}
        with tempfile.TemporaryDirectory() as d:
            tools = {t.__name__: t for t in build(case, Path(d), lambda *a, **k: None).sub_agents[0].tools}
            ctx = SimpleNamespace(state={'audio_evidence': {'source-evidence': ev}}, actions=SimpleNamespace())
            row = {'event_id': ev['inventory']['events'][0]['id'], 'audio_evidence_id': 'source-evidence', 'kind': 'impulse', 'onset_range_s': [0.6, 0.7], 'observation': 'Provisional source attack'}
            ...
            self.assertEqual(ctx.state['inventory_reviews'][0]['provenance'], 'controller_audio_hypothesis')
            for bad in [dict(row, audio_evidence_id='madeup'), dict(row, onset_range_s=[0, 0.1]), dict(row, event_id='wrong')]:
                self.assertIn('error', tools['reconcile_audio']('source-evidence', json.dumps([bad]), ctx))
            visual = dict(row)
            visual['frame_receipt_id'] = visual.pop('audio_evidence_id')
            self.assertIn('error', tools['reconcile_audio']('source-evidence', json.dumps([visual]), ctx))
            ctx.state['audio_evidence']['other'] = {**ev, 'receipt': {**r, 'role': 'target'}}
            self.assertIn('error', tools['reconcile_audio']('source-evidence', json.dumps([dict(row, audio_evidence_id='other')]), ctx))
    ...
    def test_arrangement_tool_rejects_invented_correspondence(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            sample = str(next(iter(__import__('orpheus.fitting', fromlist=['bank']).bank(case)))['id'])
            row = {'id': 'm1', 'target_id': 'not-real', 'source_id': sample, 'target_range_s': [1, 1.2], 'source_range_s': [0.1, 0.3], 'kind': 'impact', 'anchor': 'contact', 'disposition': 'use', 'confidence': 'uncertain', 'evidence': 'provisional'}
    ...
    @staticmethod
    def document(r):
        return {'media_id': r['media_id'], 'duration_s': r['duration_s'], 'audio_access': 'available', 'events': [{'kind': 'impulse', 'label': 'short impact', 'onset_range_s': [0.1, 0.2], 'offset_range_s': [0.3, 0.4], 'evidence': 'brief acoustic attack'}]}
    ...
    def test_transport_cache_disabled_and_failure(self):
        ...
        async def check(folder):
            ...
            async def request(receipt, data):
                ...
                return {'model': 'google/gemini-2.5-flash-lite', 'provider': 'test-provider', 'usage': {'prompt_tokens': 5}, 'choices': [{'finish_reason': 'stop', 'message': {'content': json.dumps(self.document(receipt))}}]}
            with patch.object(engine, 'request', side_effect=request):
                ...
                with patch.object(engine, 'enabled', False):
                    self.assertEqual((await engine.inspect('target', 0, 1))['status'], 'disabled')
            with patch.object(engine, 'request', side_effect=RuntimeError('secret payload')):
    ...
    def test_transport_payload_is_independent(self):
        ...
        async def check(folder):
            ...
            with patch.object(p, 'provider_config', return_value=('https://openrouter.ai/api/v1', 'private-key')), patch.object(p.httpx, 'AsyncClient', side_effect=lambda **kw: original(transport=httpx.MockTransport(handler), **kw)):
            ...
            self.assertEqual(p.base64.b64decode(payload[1]['input_audio']['data']), data)
    ...
    def test_signal_conflict_and_metric_units(self):
        ...
        evidence = {'evidence_id': 'test', 'receipt': r, 'inventory': inv, 'measurements': p.measurements(a * 0, r)}
    ...
    def test_reconciliation_citations_and_conflicts(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            ev = {'evidence_id': 'test', 'receipt': r, 'inventory': p.validate_inventory(self.document(r), r), 'measurements': p.measurements(a, r)}
            ctx = SimpleNamespace(state={'audio_evidence': {'test': ev}}, actions=SimpleNamespace())
            row = {'event_id': ev['inventory']['events'][0]['id'], 'frame_receipt_id': 'fake', 'kind': 'transition', 'onset_range_s': [0.6, 0.7], 'observation': 'hypothesis'}
            self.assertIn('error', tools['reconcile_audio']('test', json.dumps([row]), ctx))
            ctx.state['delivered_frame_receipts'] = [{'id': 'fake', 'video_sha256': p.digest(case['video_path'].read_bytes()), 'frames': [{'time_s': 0}, {'time_s': 1}]}]
            result = tools['reconcile_audio']('test', json.dumps([row]), ctx)
            self.assertIn('Visual and acoustic onset ranges disagree', result['events'][0]['conflicts'])
            self.assertFalse(result['events'][0]['verified'])
    ...
    def test_budget_malformed_and_no_source_audio(self):
        ...
        async def check(folder):
            logs = []
            engine = p.Perception(self.case, folder, lambda e, **k: logs.append((e, k)))
            with patch.object(engine, 'request', return_value={'model': p.MODEL, 'choices': [{'finish_reason': 'stop', 'message': {'content': 'not json secret'}}]}):
                result = await engine.inspect('target', 0, 1)
                self.assertEqual(result['status'], 'failed')
                self.assertNotIn('secret', json.dumps(logs))
                self.assertEqual(list((folder / 'perception').glob('*.json')), [])
                engine.calls = 6
                self.assertEqual((await engine.inspect('source', 0, 1))['status'], 'budget_exhausted')
            engine.case = {**self.case, 'has_original_audio': False}
            self.assertEqual((await engine.inspect('target', 0, 1))['status'], 'unavailable')
        with tempfile.TemporaryDirectory() as d:
            asyncio.run(check(Path(d)))
    ...
    def test_frame_window_bounds_and_delivery(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn('error', tools['review_window'](0, 0.5, 0.25, [0.8, 0, 0.5, 1], ctx))

    def test_regular_hypotheses_and_no_verification_bypass(self):
        from orpheus.workflow import build
        from orpheus.workflow_common import perception_gate
        r, _, a = p.window(self.case, 'target', 0, 3)
        doc = self.document(r)
        doc['events'] = [{**doc['events'][0], 'onset_range_s': [t, t + 0.02], 'offset_range_s': [t + 0.04, t + 0.08]} for t in [0.1, 0.6, 1.1, 1.6]]
        ev = {'evidence_id': 'test', 'receipt': r, 'inventory': p.validate_inventory(doc, r), 'measurements': p.measurements(a, r)}
        report = p.reconcile(ev, [], [])
        self.assertTrue(all(('Highly regular' in str(e['conflicts']) for e in report['events'])))
        state = {'audio_attempts': {'target': 'ok', 'source': 'ok'}, 'audio_reconciliations': {'test': report}}
        self.assertIsNone(perception_gate(state, True))
        self.assertIsNone(perception_gate(state, False))
        with tempfile.TemporaryDirectory() as d:
            agent = build({**self.case, 'context': '', 'style': ''}, Path(d), lambda *a, **k: None)
            tools = {t.__name__: t for t in agent.sub_agents[0].tools}
            ctx = SimpleNamespace(state={}, actions=SimpleNamespace())
            self.assertIn('error', tools['remember_decision']('Verified continuous operation', ctx))
    ...
    def test_cache_invalidation_and_http_failure(self):
        ...
        async def check(folder):
            ...
            async def request(r, data):
                return {'model': p.MODEL, 'choices': [{'finish_reason': 'stop', 'message': {'content': json.dumps(self.document(r))}}]}
            with patch.object(engine, 'request', side_effect=request) as call:
                ...
                with patch.object(p, 'PROMPT', p.PROMPT + ' Different prompt.'):
                    self.assertFalse((await engine.inspect('target', 0, 1))['cache_hit'])
                await engine.inspect('source', 0, 1)
                self.assertEqual(call.call_count, 3)
            resp = httpx.Response(403, request=httpx.Request('POST', 'https://openrouter.ai/api/v1/chat/completions'))
            with patch.object(engine, 'request', side_effect=httpx.HTTPStatusError('secret', request=resp.request, response=resp)):
                result = await engine.inspect('source', 1, 2)
                self.assertEqual(result['status_code'], 403)
            with patch.object(engine, 'request', side_effect=TimeoutError('secret')):
                self.assertEqual((await engine.inspect('source', 1, 2))['error_type'], 'TimeoutError')
        with tempfile.TemporaryDirectory() as d:
            asyncio.run(check(Path(d)))
    ...
    def test_conflicting_evidence_reaches_both_renderers(self):
        ...
        for renderer in ('render_plan', 'render_texture'):
            with tempfile.TemporaryDirectory() as d:
                agent = build(case, Path(d), lambda *a, **k: None)
                tools = {t.__name__: t for t in agent.sub_agents[0].tools}
                state = {'cycle': 1, 'inspected': True, 'plan': [{'test': 'render boundary'}], 'audio_attempts': {'target': 'ok', 'source': 'ok'}, 'audio_reconciliations': {'conflict': {'status': 'unresolved'}}, 'delivered_centers': [0, 1, 2], 'event_reviews': [{'center_s': t, 'verdict': 'uncertain'} for t in [0, 1, 2]]}
                ctx = SimpleNamespace(state=state, actions=SimpleNamespace())
                module = 'orpheus.fitting.render' if renderer == 'render_plan' else 'orpheus.texture.render'
                with patch(module, return_value={'id': 'real-render-id', 'audio_sha256': 'test-hash'}) as render:
                    result = tools[renderer](-22, 'balanced', 'Provisional', ctx) if renderer == 'render_plan' else tools[renderer](0, 2, 0, 2, -22, False, 0.1, 'Provisional', ctx)
                render.assert_called_once()
                self.assertEqual(result['perception_warnings']['unresolved_evidence_ids'], ['conflict'])
    ...
    def test_signal_role_identity_and_failure_codes(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(result['receipt']['file_sha256'], p.digest(self.case['sfx_path'].read_bytes()))
            ...
            self.assertTrue(tools['remember_decision']('This is not verified; timing remains uncertain.', ctx)['saved'])
        self.assertEqual(p.failure_code(ValueError('Wrong media identity')), 'wrong_media')
        self.assertEqual(p.failure_code(ValueError('Audio modality unavailable')), 'audio_unavailable')
