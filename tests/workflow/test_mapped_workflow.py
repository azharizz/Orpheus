from orpheus.agent import workflow_common
...
def fixture(case):
    ...
    for role in ("target", "source"):
        ...
        raw = {
            ...
            "events": [
                {
                    "kind": "impulse",
                    "label": "synthetic test event",
                    "onset_range_s": [0.1, 0.2],
                    "offset_range_s": [1.5, 1.6],
                    "evidence": "Synthetic fixture, not a model conclusion",
                }
            ],
        }
        key = p.digest(
        ...
        ev = {
            "evidence_id": key,
            "receipt": receipt,
            "served_model": p.MODEL,
            "inventory": p.validate_inventory(raw, receipt),
            "measurements": p.measurements(samples, receipt),
        }
        evidence[key] = ev
    ...
    row = {
        ...
        "target_range_s": [0.5, 1.1],
        "source_range_s": [0.1, 0.7],
        "kind": "impact",
        "anchor": "start",
        "disposition": "use",
...
class MappedWorkflow(unittest.TestCase):
    def test_stage_action_is_atomic_and_keeps_impact_gate(self):
        ...
        phases = [
            {
                "kind": "impact",
                "target_range_s": [0.5, 0.8],
                "source_range_s": [0.1, 0.4],
                "target_anchor_s": 0.65,
                "source_anchor_s": 0.2,
                "evidence": "Synthetic attack",
            },
            {
                "kind": "texture",
                "target_range_s": [0.8, 1.1],
                "source_range_s": [0.4, 0.7],
                "target_anchor_s": 0.8,
                "source_anchor_s": 0.4,
        ...
        with tempfile.TemporaryDirectory() as d:
            tools = {
                t.__name__: t
                for t in build(case, Path(d), lambda *a, **kw: None).sub_agents[0].tools
            }
            ctx = SimpleNamespace(
                state={"audio_evidence": evidence}, actions=SimpleNamespace()
            )
            ...
            self.assertIn(
                ...
                tools["render_arrangement"]("unfitted stages", ctx)["error"],
            )
    ...
    def test_coverage_and_decoded_candidate_observer(self):
        ...
        with (
            ...
            patch.object(p.Perception, "request", fake_request),
            patch.dict(os.environ, {"ORPHEUS_AUDIO_ENABLED": "1"}),
        ):
            folder = Path(d)
            logs = []
            tools = {
                t.__name__: t
                for t in build(case, folder, lambda event, **kw: logs.append(event))
                .sub_agents[0]
                .tools
            }
            ctx = SimpleNamespace(
                state={
                    ...
                    "cycle": 1,
                    "candidates": [],
                    "delivered_centers": [0.8],
                    "event_reviews": [
                        {"center_s": 0.8, "verdict": "target", "observation": "Fixture"}
                    ],
                },
            ...
            report = asyncio.run(
                tools["review_candidate_audio"](rendered["id"], 0, 1, ctx)
            )
            self.assertEqual(report["status"], "ok")
            self.assertEqual(
                report["receipt"]["file_sha256"],
                p.digest((folder / (rendered["id"] + ".mp4")).read_bytes()),
            )
    ...
    def test_temporal_review_delivery_accumulates_across_windows(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch.object(
                p, "window", side_effect=ValueError("Shorter original audio")
            ):
    ...
    def test_automatic_fitting_tools_persist_mapping(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn(
                ...
                tools["fit_mapping_impact"](
                    "m1", 0, 0.8, "Missing source inspection", ctx
                )["error"],
            )
            options = tools["inspect_mapping_source"]("m1", ctx)
            self.assertGreater(options["total"], 0)
            result = tools["fit_mapping_impact"](
                "m1",
                0,
                0.8,
                "Synthetic integration target contact, not visual truth",
                ctx,
            )
            self.assertNotIn("error", result)
            self.assertEqual(
                ctx.state["arrangement"]["rows"][0]["target_anchor_s"], 0.8
            )
            self.assertEqual(
                ctx.state["arrangement"]["rows"][0]["target_body_dbfs"], -22
            )
            ...
            ctx.state["event_reviews"] = [
                {
                    "center_s": 0.8,
                    "verdict": "target",
                    "observation": "Synthetic temporal review",
                }
            ]
            ready = tools["render_arrangement"](
                "after contact, source and temporal evidence", ctx
            )
            self.assertNotIn("error", ready)
            self.assertIn(
                "error",
                tools["fit_mapping_impact"]("m1", 999, 0.8, "invalid option test", ctx),
            )
    ...
    def test_compact_mapping_context_separates_arrangement_and_row_ids(self):
        ...
        second = {
            **row,
            "id": "m2",
            "target_range_s": [1.1, 1.6],
            "source_range_s": [0.1, 0.7],
            "reuse_reason": "Synthetic fixture reuses the same inspected source event",
        }
        with tempfile.TemporaryDirectory() as d:
            editor = build(case, Path(d), lambda *a, **kw: None).sub_agents[0]
            tools = {t.__name__: t for t in editor.tools}
            ...
            self.assertNotEqual(
                context["arrangement_id"], context["valid_mapping_ids"][0]
            )
    ...
    def test_batch_inspection_and_fitting_preserve_per_row_receipts(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertNotIn(
                "error", tools["save_arrangement"](json.dumps([row, second]), ctx)
            )
                ...
                [
                    {
                        "mapping_id": "m1",
                        "source_option": 0,
                        "target_contact_s": 0.8,
                        "reason": "Synthetic first contact",
                    },
                    {
                        "mapping_id": "m2",
                        "source_option": 0,
                        "target_contact_s": 1.3,
                        "reason": "Synthetic second contact",
                    },
                    {
                        "mapping_id": "missing",
                        "source_option": 0,
                        "target_contact_s": 1.4,
                        "reason": "Invalid row test",
                    },
                ]
            )
            ...
            ctx.state["event_reviews"] = [
                {
                    "center_s": 0.8,
                    "verdict": "target",
                    "observation": "Synthetic first contact",
                },
                {
                    "center_s": 1.3,
                    "verdict": "target",
                    "observation": "Synthetic second contact",
                },
            ]
            ...
            self.assertEqual(
                [x["mapping_id"] for x in measured["fitted_impacts"]], ["m1", "m2"]
            )
    ...
    def test_prompt_assets_are_owned_by_package(self):
        ...
        self.assertEqual(
            workflow.MAPPED_INSTRUCTION,
            (prompt_dir / "controller-mapped.md").read_text(),
        )
        self.assertEqual(
            workflow.INSTRUCTION, (prompt_dir / "controller-fallback.md").read_text()
        )
        self.assertEqual(
            workflow.PERCEPTION_INSTRUCTION, (prompt_dir / "perception.md").read_text()
        )
    ...
    def test_runtime_prompt_assets_render_context_and_frame_labels(self):
        ...
        self.assertEqual(
            set(workflow_common.RUNTIME_PROMPTS),
            {
                "project_state",
                "completion_reserve",
                "runtime_status",
                "strategy_reserve_few",
                "strategy_reserve_many",
                "recovery",
                "frame_label",
                "user_request",
            },
        ...
        self.assertTrue(
            workflow_common.runtime_prompt(
                "project_state", snapshot='{"ok":true}'
            ).startswith("Project state: ")
        )
        self.assertIn(
            "Remaining calls: 7",
            workflow_common.runtime_prompt("completion_reserve", remaining=7),
        )
        self.assertEqual(
            workflow_common.runtime_prompt("frame_label", timestamp="1.2500"),
            "Clipped-video frame timestamp 1.2500 seconds",
        )
        values = {
            "project_state": {"snapshot": '{"notes":["{literal}"]}'},
            "completion_reserve": {"remaining": 7},
            "runtime_status": {"cycle": 2, "renders": 1, "notes": ["{literal}"]},
            "strategy_reserve_few": {},
            "strategy_reserve_many": {"count": 2},
            "recovery": {"action": "inspect row_1"},
            "frame_label": {"timestamp": "1.2500"},
            "user_request": {
                "feedback": "Keep {literal} text",
                "context": "a machine",
                "style": "subtle",
                "seconds": 30,
            },
            ...
            with self.subTest(template=name):
                fields = {
                    ...
                    for _, field, _, _ in Formatter().parse(
                        workflow_common.RUNTIME_PROMPTS[name]
                    )
        ...
        self.assertTrue(
            ...
            ).startswith("User request:")
        )
        self.assertIn(
            "Keep {literal} text",
            workflow_common.runtime_prompt("user_request", **values["user_request"]),
        )
        with tempfile.NamedTemporaryFile(suffix=".jpg") as image:
            ...
            self.assertEqual(
                parts[0].text, "Clipped-video frame timestamp 1.2500 seconds"
            )
    ...
    def test_40_call_budget_and_reserve(self):
        ...
        self.assertEqual(
            RunConfig(max_llm_calls=worker.MAX_CONTROLLER_CALLS).max_llm_calls, 40
        )
    ...
    def test_unsuitable_sentinel_and_budget_callbacks(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIsNone(
                editor.before_tool_callback(
                    SimpleNamespace(name="inspect_scene"), {}, ctx
                )
            )
            ...
            self.assertIsNone(
                editor.before_tool_callback(
                    SimpleNamespace(name="render_arrangement"), {}, ctx
                )
            )
            for _ in range(3):
                r = editor.after_tool_callback(
                    SimpleNamespace(name="save_arrangement"),
                    {},
                    ctx,
                    {"error": "bad ID"},
                )
            ...
            result = tools["finish"](
                "none", "unsuitable", "No suitable source content", [], ctx
            )
            self.assertNotIn("error", result)
    ...
    def test_real_adk_two_renders_resume_and_unsuitable(self):
        ...
        class Scripted(BaseLlm):
            ...
            async def generate_content_async(self, llm_request, stream=False):
                ...
                actions = [
                    ("save_arrangement", {"rows_json": json.dumps([row])}),
                    ("inspect_mapping_source", {"mapping_id": "m1"}),
                    (
                        "fit_mapping_impact",
                        {
                            "mapping_id": "m1",
                            "source_option": 0,
                            "target_contact_s": 0.8,
                            "reason": "Synthetic contact alignment test",
                        },
                    ),
                    (
                        "review_window",
                        {"start_s": 0.7, "end_s": 0.9, "step_s": 0.1, "crop": []},
                    ),
                    (
                        "record_event_review",
                        {
                    ...
                    (
                        "render_arrangement",
                        {
                            "hypothesis": "Synthetic baseline with automatic impact preparation"
                        },
                    ),
                    (
                        "measure_candidate",
                    ...
                    (
                        "revise_mapping_gain",
                        {
                            "mapping_id": "m1",
                            "gain_db": -4,
                            "reason": "Measured baseline needs lower level",
                        },
                    ),
                    (
                        "render_arrangement",
                        {"hypothesis": "Lower gain, preserve mapping evidence"},
                    ),
                    (
                        "measure_candidate",
                        {"candidate_id": candidates[-1]["id"] if candidates else ""},
                    ...
                    (
                        "finish",
                        {
                            ...
                            "comparison": "Synthetic -4dB revision measured; no listening claim",
                            "unresolved": [
                ...
                part = (
                    types.Part(text="Next cycle")
                    if action is None
                    else types.Part(
                        function_call=types.FunctionCall(
                            name=action[0], args=action[1], id=str(n)
                        )
                    )
                )
        ...
        async def run(folder):
            service = DatabaseSessionService(
                db_url="sqlite+aiosqlite:///" + str(folder / "test.sqlite")
            )
            ...
            async for _ in runner.run_async(
                ...
                new_message=types.Content(
                    role="user",
                    parts=[types.Part(text="User request: synthetic integration")],
                ),
            ):
                pass
            session = await service.get_session(
                app_name="test", user_id="local", session_id="one"
            )
            self.assertEqual(len(session.state["candidates"]), 2)
            self.assertEqual(
                session.state["arrangement"]["rows"][0]["target_anchor_s"], 0.8
            )
            self.assertEqual(
                session.state["arrangement"]["rows"][0]["target_body_dbfs"], -22
            )
            self.assertEqual(
                session.state["selection"]["decision"], "needs_human_review"
            )
            self.assertNotEqual(
                *[x["audio_sha256"] for x in session.state["candidates"]]
            )
            self.assertFalse(
                session.state["candidates"][1]["strategy"]["distinct_from_prior"]
            )
            await runner.close()
            await service.close()
            reopened = DatabaseSessionService(
                db_url="sqlite+aiosqlite:///" + str(folder / "test.sqlite")
            )
            ...
            fresh = await reopened.create_session(
                app_name="test", user_id="local", session_id="two", state={}
            )
        ...
        if os.environ.get("ORPHEUS_TEST_TRACE"):
            from orpheus.domain.projects import atomic

            atomic(
                Path(os.environ["ORPHEUS_TEST_TRACE"]),
                {
                    "type": "synthetic_controller_real_adk_real_dsp",
                    "model_accuracy_test": False,
                    "events": logs,
                },
            )
